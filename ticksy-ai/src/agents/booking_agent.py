...
from src.instructions.booking_agent_instruction import get_booking_agent_system_prompt
...
class BookingAgent:
    """Booking agent built with LangGraph for handling ticket bookings."""
    
    def __init__(self, model=None):
        self.name = "BookingAgent"
        self.model = model or OpenAILLM().get_llm_model()
        self.instructions = get_booking_agent_system_prompt()
        
        # Build the graph
        self.graph = self._build_graph()
    
    def _build_graph(self) -> StateGraph:
        """Build the LangGraph graph for booking agent."""
    ...
    def _agent_node(self, state: BookingAgentState) -> BookingAgentState:
        ...
        try:
            ...
            logger.error(f"Error in booking agent node: {e}", exc_info=True)
            error_message = AIMessage(content=f"I apologize, but I encountered an error. Please try again.")
